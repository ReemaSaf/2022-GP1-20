...
class StationsController extends GetxController{
  ...
  get_bus_stations() async {
    ...
    var result =await FirebaseFirestore.instance.collection("Stations").where("type", isEqualTo: "Bus").get();
  ...
  get_Metro_stations() async {
    stations_loading(true);
    update();
    var result =await FirebaseFirestore.instance.collection("Stations").where("Type", isEqualTo: "Metro").get();
    print("here is result data ${result.docs.length}");
